...
const tabs = [
  ...
  { slug: "simulation-evidence", label: "Simulation Evidence" },
  { slug: "fmi", label: "FMI" },
  { slug: "links", label: "Registry" },
...
export default async function ProjectWorkspace({ params, searchParams }: { params: { id: string; section?: string[] }; searchParams?: { view?: string; focus?: string; selected?: string; kind?: string; relation?: string; evidence?: string; layer?: string } }) {
    ...
    return (
      <div className="space-y-6">
        ...
        {view === "mapping-contract" ? (
          <MappingContractView contract={mappingContract} />
        ) : view === "satisfaction" ? (
  ...
  if (section === "blocks") {
    ...
    return (
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader><div className="font-semibold">Blocks</div></CardHeader>
          <CardBody>
                  ...
                  <Link key={item.key} href={`/blocks/${item.id}`} className={`block rounded-xl border p-4 hover:border-accent/50 ${item.abstraction_level === "physical" ? "border-amber-400/30 bg-amber-500/5" : "border-sky-400/30 bg-sky-500/5"}`}>
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        ...
                        <div className="text-xs text-muted">{item.block_kind} · v{item.version}</div>
                      </div>
  ...
  if (section === "operational-evidence") {
    return (
      <div className="space-y-6">
        <SectionTitle title={`${project.code} - Operational Evidence`} description="Batch-style operational feedback linked to requirements and verification evidence." />
        <Tabs projectId={project.id} section={section} />
        <Card>
          <CardHeader><div className="font-semibold">Operational evidence batches</div></CardHeader>
        ...
        <Card>
          <CardHeader><div className="font-semibold">Add operational evidence</div></CardHeader>
          <CardBody>
  ...
  if (section === "simulation-evidence") {
    ...
    return (
      <div className="space-y-6">
        <SectionTitle title={`${project.code} - Simulation Evidence`} description="First-class simulation evidence records linked to requirements, tests, and verification evidence." />
        <Tabs projectId={project.id} section={section} />
        <Card>
          <CardHeader><div className="font-semibold">Simulation evidence records</div></CardHeader>
          <CardBody className="space-y-4">
        ...
        <Card>
          <CardHeader><div className="font-semibold">Add simulation evidence</div></CardHeader>
          <CardBody>
                  ...
                  <Link key={contract.id} href={`/fmi-contracts/${contract.id}`} className="block rounded-xl border border-line bg-panel2 p-4 hover:border-accent/50">
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        ...
                        <div className="text-xs text-muted">{contract.model_identifier} Â· {contract.contract_version}</div>
                      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        ...
        <Card>
          ...
          <CardBody className="space-y-3">
            ...
            <Button href={`/projects/${project.id}/links`} className="w-full" variant="secondary">Relationship registry</Button>
...
function MappingContractView({ contract }: { contract: SysMLMappingContractResponse | null }) {
  if (!contract) {
      ...
      <Card>
        ...
        <CardBody className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <Mini metric="Requirements" value={contract.summary.requirement_count} />
            <Mini metric="Blocks" value={contract.summary.block_count} />
                    ...
                    <div className="space-y-2">
                      {row.linked_cad_artifacts.map((artifact) => (
                        <Link key={artifact.id} href={`/external-artifacts/${artifact.id}`} className="block rounded-lg border border-line bg-background p-3 text-sm hover:border-accent/50">
                          ...
                          <div className="text-xs text-muted">{artifact.connector_name || "No connector"} · {artifact.artifact_type} · versions {(artifact.versions || []).length}</div>
                        </Link>
                      ))}
                    </div>
                  ) : (
